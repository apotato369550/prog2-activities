...
void sortList(personRecord *record);
void insertSorted(personRecord *record, person insert);
	...
	while (input != 0) {
		...
		switch (input) {
			...
			case 8:
				...
				scanf("%s", name);
				printf("Enter age of  person to delete: ");
				scanf("%d", &age);
				strcpy(insert.name, name);
				insert.age = age;
...
void sortList(personRecord *record) {
	for (int i = 0; i < record->count; i++) {
		for (int j = 0; j < record->count - i - 1; j++) {
			if (record->personList[j].age > record->personList[j + 1].age) {
				person temp = record->personList[j];
				record->personList[j] = record->personList[j + 1];
				record->personList[j + 1] = temp;
			}
		}
	}
...
void insertSorted(personRecord *record, person insert) {
	...
	int index = 0;
	for (int i = 0; i < record->count; i++) {
		if (record->personList[i].age < insert.age) {
			index++;
		}
	}
	insertAtIndex(record, insert, index);
